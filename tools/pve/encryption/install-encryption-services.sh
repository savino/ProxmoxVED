...
if [ "$SERVICE_TYPE" = "ct" ] || [ "$SERVICE_TYPE" = "both" ]; then
	if install_file tools/pve/encryption/zfs-load-lxc-keys.sh "$DEST_SCRIPT" 700; then
		installed_any=1
	fi
	if install_file tools/pve/encryption/zfs-load-lxc-keys.service "$DEST_UNIT1" 644; then
		INST_UNIT1=1; installed_any=1
	fi
fi

if [ "$SERVICE_TYPE" = "global" ] || [ "$SERVICE_TYPE" = "both" ]; then
	if install_file tools/pve/encryption/zfs-load-all-keys-from-usb.service "$DEST_UNIT2" 644; then
...
case "$SERVICE_TYPE" in
	global|both)
		if [ "$INST_UNIT2" -eq 1 ]; then
			printf 'Enabling %s\n' "$(basename "$DEST_UNIT2")"
			sudo systemctl enable --now "$(basename "$DEST_UNIT2")" || true
		fi
		;;&
	ct|both)
...
# validate SERVICE_TYPE (after die() is defined)
case "$SERVICE_TYPE" in
	global|ct|both) ;;
